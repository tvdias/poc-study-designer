...
using Kantar.StudyDesignerLite.Plugins.StudySnapshotLineChangelog;
using Kantar.StudyDesignerLite.Plugins.Tests.EntityBuilders;

namespace Kantar.StudyDesignerLite.Plugins.Tests.Plugins.StudySnapshotLineChangelog
{
    [TestClass]
    public class StudySnapshotLineChangelogPreValidationTests
    {
        ...
        public void CreateOrUpdate_ShouldThrow_WhenStudyNotDraft()
        {
            ...
            // Arrange StudySnapshotLineChangelog
            var changelog = new StudySnapshotLineChangelogBuilder(study)
                .Build();

            _context.Initialize(new List<Entity> { study });
            var pluginContext = _context.GetDefaultPluginContext();
            pluginContext.MessageName = "Create";
            pluginContext.InputParameters["Target"] = changelog;
            // Act & Assert
            try
            {
                // Act
                _context.ExecutePluginWith<StudySnapshotLineChangelogPreValidation>(pluginContext);
        ...
        {
            var changelog = new StudySnapshotLineChangelogBuilder(null)
                .Build();

            // Remove the study reference to simulate the test case
            changelog.Attributes.Remove(KTR_StudySnapshotLineChangelog.Fields.KTR_CurrentStudy);
            ...
            Assert.IsFalse(changelog.Attributes.ContainsKey(KTR_StudySnapshotLineChangelog.Fields.KTR_CurrentStudy));
        }
        ...
        public void Execute_ShouldReturn_WhenNoStudyReferenceInPreImage()
        {
            var changelog = new StudySnapshotLineChangelogBuilder(null)
                .Build();
            var preImage = new StudySnapshotLineChangelogBuilder(null)
                .Build();
            ...
            Assert.IsFalse(changelog.Attributes.ContainsKey(KTR_StudySnapshotLineChangelog.Fields.KTR_CurrentStudy));
            Assert.IsFalse(preImage.Attributes.ContainsKey(KTR_StudySnapshotLineChangelog.Fields.KTR_CurrentStudy));
        }
        ...
        public void Execute_ShouldReturn_WhenStudyHasNoStatusCode()
        {
            ...
            study.StatusCode = null; // Explicitly set to null to simulate no status code
