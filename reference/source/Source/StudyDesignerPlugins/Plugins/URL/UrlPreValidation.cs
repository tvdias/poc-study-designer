...
namespace Kantar.StudyDesignerLite.Plugins.Url
{
    public class UrlPreValidation : PluginBase
    {
        private const string PluginName = "Kantar.StudyDesignerLite.Plugins.UrlPreValidation";
        public UrlPreValidation()
            : base(typeof(UrlPreValidation))
        {
        }
        ...
        protected override void ExecuteCdsPlugin(ILocalPluginContext localPluginContext)
        {
            ...
            // Get the study reference from the URL entity
            EntityReference studyRef = null;
            if (target.Contains(KTR_Url.Fields.KTR_Study))
            {
                studyRef = target[KTR_Url.Fields.KTR_Study] as EntityReference;
            }
            else if (context.MessageName == nameof(ContextMessageEnum.Update) && context.PreEntityImages.Contains("PreImage"))
            {
                var preImage = context.PreEntityImages["PreImage"];
                if (preImage.Contains(KTR_Url.Fields.KTR_Study))
                {
                    studyRef = preImage[KTR_Url.Fields.KTR_Study] as EntityReference;
                }
            }
