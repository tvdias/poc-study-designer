...
{
    public class QuestionnaireLineQLMLESyncPostOperation : PluginBase
    {
        private const string PluginName =
            "Kantar.StudyDesignerLite.Plugins.QuestionnaireLine.QuestionnaireLineQLMLESyncPostOperation";

        public QuestionnaireLineQLMLESyncPostOperation()
            : base(typeof(QuestionnaireLineQLMLESyncPostOperation)) { }
        ...
        protected override void ExecuteCdsPlugin(ILocalPluginContext localContext)
        {
            ...
            var stateCode = preImage.GetAttributeValue<OptionSetValue>(
                KT_QuestionnaireLines.Fields.StateCode)?.Value;

            var preState =
            preImage.GetAttributeValue<OptionSetValue>(KT_QuestionnaireLines.Fields.StateCode)?.Value;

            var postState = context.InputParameters.Contains("Target")
                ? ((Entity)context.InputParameters["Target"])
                    .GetAttributeValue<OptionSetValue>(KT_QuestionnaireLines.Fields.StateCode)?.Value
                : (int?)null;

            // Deactivate = only for Update when Active -> Inactive
            bool isDeactivate =
                preState == (int)KT_QuestionnaireLines_StateCode.Active &&
                postState == (int)KT_QuestionnaireLines_StateCode.Inactive;

            if (!isDeactivate)
            {
                tracing.Trace("Not a deactivate – exiting.");
                return;
            }

            tracing.Trace($"Processing DEACTIVATE for QuestionnaireLine {preImage.Id}");
            ...
            // 1. Deactivate QLMLEs
            var deactivatedQLMLEs = DeactivateQLMLEs(
                preImage.Id,
                qlmleRepo,
                tracing);
        ...
        private List<KTR_QuestionnaireLinemanAgedListEntity> DeactivateQLMLEs(
            Guid questionnaireLineId,
            QuestionnaireLineManagedListEntityRepository qlmleRepo,
            ITracingService tracing)
        {
            tracing.Trace($"Fetching QLMLEs for QuestionnaireLine {questionnaireLineId}");

            var qlmles = qlmleRepo.GetDraftStudyQLMLEsByQuestionnaireLineId(questionnaireLineId);
            ...
            ITracingService tracing)
        {
            ...
            foreach (var group in studyMleGroups)
            {
                if (!qlmleRepo.HasActiveQLMLEs(group.Key.StudyId, group.Key.MleId))
                {
                    ...
                    if (studyMle != null)
                    {
                        ...
                        tracing.Trace(
                            $"StudyMLE {studyMle.Id} queued for deactivation.");
                    }
                }
            }
            ...
            if (studyMlesToDeactivate.Any())
            {
                ...
                tracing.Trace(
                    $"Deactivated {studyMlesToDeactivate.Count} StudyManagedListEntity records.");
            }
        }
