...
// </copyright>
// <author></author>
// <date>3/2/2025 11:02:59 PM</date>
...
namespace Kantar.StudyDesignerLite.Plugins
{
    ...
    public abstract class PluginBase : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            {
                LogException(localPluginContext, ex);
        ...
        private void LogException(
            ...
            Exception ex)
        {
            var exceptionTypeMessage = string.Empty;
            switch (ex)
            {
                case FaultException<OrganizationServiceFault>_:
                    exceptionTypeMessage = "OrganizationServiceFault Exception";
                    break;
                case InvalidPluginExecutionException _:
                    exceptionTypeMessage = "Expected Exception";
                    break;
                default:
                    exceptionTypeMessage = "Unexpected Exception";
                    break;
            }

            var logMessage =
               $"----> {exceptionTypeMessage} IN PLUGIN: {PluginClassName} " +
               $"| CORRELATION_ID: {localPluginContext.PluginExecutionContext.CorrelationId} " +
               $"| MESSAGE_NAME: {localPluginContext.PluginExecutionContext.MessageName}";

            if (ex.InnerException != null)
            {
                logMessage = $"{logMessage} | INNER EX: {ex.InnerException.Message}";
            }

            localPluginContext.Trace($"{logMessage} | EXCEPTION: {ex}");

            localPluginContext.Logger?.LogError(ex, logMessage);
        }
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        public LocalPluginContext(IServiceProvider serviceProvider)
        {
            ...
            // This will automatically route to Application Insights if configured
            try
            {
