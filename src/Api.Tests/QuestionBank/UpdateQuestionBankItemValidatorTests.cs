...
public class UpdateQuestionBankItemValidatorTests
{
    private readonly UpdateQuestionBankItemValidator _validator = new();
    ...
    public async Task VariableNameTooLong_ShouldFailValidation()
    {
        ...
        // Assert
        Assert.False(result.IsValid);
        Assert.Contains(result.Errors, e => e.ErrorMessage == "Variable Name must not exceed 200 characters");
    }
