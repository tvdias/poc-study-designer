...
import type { CommissioningMarket } from '../services/api';
...
export function CommissioningMarketsPage() {
    const [markets, setMarkets] = useState<CommissioningMarket[]>([]);
    const [search, setSearch] = useState('');
    const [isLoading, setIsLoading] = useState(true);

    // Panel State
    const [mode, setMode] = useState<Mode>('list');
    const [selectedMarket, setSelectedMarket] = useState<CommissioningMarket | null>(null);
    ...
    const fetchMarkets = async () => {
        setIsLoading(true);
        try {
            const data = await commissioningMarketsApi.getAll(search);
            setMarkets(data);
        } catch (error) {
            console.error('Failed to fetch commissioning markets', error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openView = (market: CommissioningMarket) => {
        setSelectedMarket(market);
        setMode('view');
    };

    const openEdit = (market?: CommissioningMarket) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.status === 400 && err.errors) {
                ...
                setServerError(err.detail || "Commissioning Market already exists");
            } else {
    ...
    const handleDelete = async (market?: CommissioningMarket) => {
        const target = market || selectedMarket;
        if (!target || !confirm(`Are you sure you want to delete commissioning market '${target.name}'?`)) return;

        try {
            await commissioningMarketsApi.delete(target.id);
            closePanel();
            fetchMarkets();
        } catch (error) {
            console.error('Failed to delete commissioning market', error);
        }
    };
        ...
        <div className="page-container">
            ...
            <div className="command-bar card">
                ...
                <div className="search-box">
                    <input
                        type="text"
                        placeholder="Search commissioning markets..."
                        value={search}
                ...
                {isLoading ? (
                    ...
                    <table className="details-list">
                        ...
                        <tbody>
                            ...
                            {markets.length === 0 && (
                                <tr><td colSpan={4} className="empty-state">No commissioning markets found.</td></tr>
                            )}
                        </tbody>
                    </table>
                )}
            ...
            <SidePanel
                isOpen={mode !== 'list'}
                onClose={closePanel}
                title={mode === 'create' ? 'New Commissioning Market' : mode === 'edit' ? 'Edit Commissioning Market' : selectedMarket?.name || 'Commissioning Market Details'}
                footer={
